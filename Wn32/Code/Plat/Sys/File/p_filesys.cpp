...
namespace File {
    ...
    static std::filesystem::path GetModulePath() {
        // There's no any platform-agnostic way of doing so...
        // Well, cross-platform stuff is usually about hiding platform-specific stuff...
        // Whatever. *depressed noises*
#ifdef __PLAT_WN32__
        // Get module namethe
...
#	endif
    ...
    int Seek(void *pFP, long offset, int origin) {
        ...
        sFileHandle *h_file = (sFileHandle *) pFP;
        h_file->file.seekg(offset, (origin == SEEK_CUR) ? std::ios::cur : ((origin == SEEK_SET) ? std::ios::beg
                                                                                                : std::ios::end));
        return h_file->file.tellg();
    }
